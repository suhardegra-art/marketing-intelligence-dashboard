...
import { saveYouTubeChannelSnapshot } from "@/lib/youtube-supabase";
import type { YouTubePreviewContent } from "@/app/youtube/youtubePreviewData";
...
export type YouTubeDashboardData = {
  ...
  trafficSources: Array<{ label: string; views: number; percentage: number }>;
  contentTypes: Array<{ label: string; views: number; percentage: number }>;
  ages: Array<{ label: string; percentage: number }>;
  genders: Array<{ label: string; percentage: number }>;
  devices: Array<{ label: string; views: number; percentage: number }>;
  geographies: Array<{ label: string; views: number; percentage: number }>;
  subscribedStatus: Array<{ label: string; watchMinutes: number; percentage: number }>;
...
function dateDiffInclusive(fromDate: string, toDate: string) {
  const from = new Date(`${fromDate}T00:00:00Z`).getTime();
  const to = new Date(`${toDate}T00:00:00Z`).getTime();
  return Math.max(1, Math.round((to - from) / 86400000) + 1);
}

function shiftPeriodBack(fromDate: string, days: number) {
...
function numberValue(row: Record<string, string | number> | undefined, key: string) {
...
function trafficSourceLabel(value: string) {
...
function overviewFromRow(row: Record<string, string | number> | undefined) {
  ...
  return {
    views,
    watchHours: numberValue(row, "estimatedMinutesWatched") / 60,
    subscribersNet,
    likes,
    comments,
    shares,
...
export async function getYouTubeDashboardData(input?: {
  from?: string;
  to?: string;
  ...
  if (!configured) {
    return {
      connected: false,
      configured: false,
      message: "YouTube OAuth environment variables are not configured yet."
    };
  }

  let accessToken: string | null = null;
  try {
    accessToken = await getValidYouTubeAccessToken();
  } catch (error) {
    return {
      connected: false,
      configured: true,
  ...
  if (!accessToken) {
    return {
      connected: false,
      configured: true,
      message: "YouTube has not been connected yet."
    };
  }

  const defaults = defaultPeriod();
  const fromDate = input?.from || defaults.fromDate;
  const toDate = input?.to || defaults.toDate;
  const periodDays = dateDiffInclusive(fromDate, toDate);
  const previous = shiftPeriodBack(fromDate, periodDays);

  const growthEnd = toDate;
  const growthStartDate = new Date(`${growthEnd}T00:00:00Z`);
  growthStartDate.setUTCDate(growthStartDate.getUTCDate() - 364);
  const growthStart = isoDate(growthStartDate);
  ...
  const [
    ...
    safeAnalyticsRows(accessToken, {
      startDate: previous.fromDate,
      endDate: previous.toDate,
      metrics:
  ...
  const overview = overviewFromRow(overviewRows[0]);
  const previousOverviewBase = overviewFromRow(previousRows[0]);
  const previousOverview = {
    views: previousOverviewBase.views,
    watchHours: previousOverviewBase.watchHours,
    subscribersNet: previousOverviewBase.subscribersNet,
    engagementRate: previousOverviewBase.engagementRate
  };

  const daily: YouTubeDailyPoint[] = dailyRows.map((row) => ({
    date: String(row.day || ""),
    views: numberValue(row, "views"),
    watchMinutes: numberValue(row, "estimatedMinutesWatched"),
    subscribersGained: numberValue(row, "subscribersGained"),
    subscribersLost: numberValue(row, "subscribersLost"),
    likes: numberValue(row, "likes"),
    comments: numberValue(row, "comments"),
    shares: numberValue(row, "shares")
  }));

  const growthDaily: YouTubeDailyPoint[] = growthRows.map((row) => ({
    date: String(row.day || ""),
    views: numberValue(row, "views"),
    watchMinutes: numberValue(row, "estimatedMinutesWatched"),
    subscribersGained: numberValue(row, "subscribersGained"),
  ...
  const ages = ageRows.map((row) => ({
    label: ageLabel(String(row.ageGroup || "")),
    percentage: numberValue(row, "viewerPercentage")
  }));

  const genders = genderRows.map((row) => ({
    label: String(row.gender || "Unknown"),
    percentage: numberValue(row, "viewerPercentage")
  }));
  ...
  const devices = deviceRows.map((row) => {
    const views = numberValue(row, "views");
    return {
      label: deviceLabel(String(row.deviceType || "OTHER")),
      views,
      percentage: deviceTotal ? (views / deviceTotal) * 100 : 0
    };
  });
  ...
  const geographies = countryRows.map((row) => {
    ...
    return {
      ...
      views,
      percentage: countryTotal ? (views / countryTotal) * 100 : 0
    };
  });
