...
async function fetchJson<T>(url: string, accessToken: string): Promise<T> {
  ...
  try {
    ...
    throw new Error(`YouTube API returned a non-JSON response (${response.status}).`);
  }
...
export async function getMyYouTubeChannel(accessToken: string): Promise<YouTubeChannel> {
  ...
  if (!item) throw new Error("No YouTube channel is available for this Google account.");
...
export async function getVideoMetadata(
  ...
  const output = new Map<string, YouTubeVideoMetadata>();
  ...
  for (let index = 0; index < videoIds.length; index += 50) {
    ...
    }>(`https://www.googleapis.com/youtube/v3/videos?${params.toString()}`, accessToken);
...
export function analyticsRowsToObjects(report: AnalyticsReport) {
  const headers = report.columnHeaders?.map((header) => header.name) || [];
  return (report.rows || []).map((row) => {
    const result: Record<string, string | number> = {};
    headers.forEach((header, index) => {
      result[header] = row[index] ?? 0;
    });
    return result;
  });
}
...
export async function safeAnalyticsRows(
  accessToken: string,
  input: Parameters<typeof queryYouTubeAnalytics>[1]
) {
  try {
    const report = await queryYouTubeAnalytics(accessToken, input);
    return analyticsRowsToObjects(report);
  } catch (error) {
    console.warn("YouTube Analytics query skipped", input, error);
